        ...
        Alert.alert(
            ...
            [
                { text: t('cancel'), style: 'cancel' },
                {
                    ...
                    onPress: async () => {
                        try {
                            // 1. Backend'e silme isteğini gönder
                            await api.delete(`/posts/${postToDelete.id}`);

                            // 2. Başarılı olursa, fetchData() yerine state'i manuel olarak güncelle
                            setClubDetails(currentDetails => {
                                // Eğer state boşsa, bir şey yapma (güvenlik önlemi)
                                if (!currentDetails) return null;

                                // Mevcut gönderi listesinden, sildiğimiz gönderiyi çıkararak yeni bir liste oluştur
                                const updatedPosts = currentDetails.posts.filter(
                                    post => post.id !== postToDelete.id
                                );

                                // State'i, güncellenmiş gönderi listesiyle birlikte geri döndür
                                return { ...currentDetails, posts: updatedPosts };
                            });

                            // 3. Başarı uyarısını göster
                    ...
                    { text: t('cancel'), style: 'cancel' },
                    {
                        ...
                        onPress: async () => {
                            try {
                                ...
                                navigation.goBack(); // Ayrıldıktan sonra bir önceki ekrana dön
                            } catch (error) {
